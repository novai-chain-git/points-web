...
export function getBscScanLink(data: string | number | string[], type: 'transaction' | 'token' | 'address' | 'block' | 'countdown' | 'holders' | 'balance', chainId: ChainId = BASE_CHAIN_ID): string {
...
export function shortenAddress(address: string, chars = 1): string {
  ...
  return `${parsed.substring(0, chars + 5)}...${parsed.substring(39 - chars)}`;
}
...
// account is optional
export function getContract(address: string, ABI: any, signer?: Signer | ethers.providers.Provider): Contract {
